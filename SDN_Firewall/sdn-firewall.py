...
def firewall_policy_processing(policies):
    ...
    for policy in policies:
        ...
        # Match Source/Destination MAC addresses
        if policy['mac-src'] != '-':
            match.dl_src = EthAddr(policy['mac-src'])
        if policy['mac-dst'] != '-':
            match.dl_dst = EthAddr(policy['mac-dst'])

        # Match Source/Destination IP addresses
        if policy['ip-src'] != '-':
        ...
        # Create a POX Flow Modification Object
        rule = of.ofp_flow_mod()

        # Match criteria for the flow modification
        rule.match = match

        # Set the action based on the policy
