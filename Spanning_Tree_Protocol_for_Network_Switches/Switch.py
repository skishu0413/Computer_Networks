...
class Switch(StpSwitch):
    ...
    def __init__(self, idNum: int, topolink: object, neighbors: list):
        ...
        self.distance = 0
        self.activeLinks = []
        self.pathNeighbourThroughToRoot = None
    ...
    def process_message(self, message: Message):
        ...
        # new distance based on the switch received a message
        new_distance = message.distance + 1

        # update the root with a lower claimroot
        if message.root < self.root:
          self.root = message.root
          self.distance = new_distance
          self.activeLinks = [message.origin]
          self.pathNeighbourThroughToRoot = message.origin

        # check for a lower switch to update the root
        elif new_distance == self.distance:
          if message.root == self.root:
            if message.origin < self.pathNeighbourThroughToRoot:
              self.activeLinks = [message.origin]
              self.pathNeighbourThroughToRoot = message.origin

        #check for shorter distance to root and update distance
        elif new_distance < self.distance:
          if message.root == self.root:
            self.distance = new_distance
            self.activeLinks = [message.origin]
        ...
        # add originId to active links as it relies in current switch to reach the root
        if message.pathThrough:
          if message.origin not in self.activeLinks:
            self.activeLinks.append(message.origin)
        # remove originId from active links as it doesn't relies on current switch to reach the root
        elif message.origin != self.pathNeighbourThroughToRoot:
          if message.origin in self.activeLinks:
            self.activeLinks.remove(message.origin)

        # send an update to its neighbor if TTL is greater than 0
        if message.ttl > 0:
            for neighbor in self.links:
                # when pathThrough should only be TRUE
                if neighbor == self.pathNeighbourThroughToRoot:
                    new_message = Message(self.root, self.distance,
                                        self.switchID, neighbor,
                                        True, message.ttl - 1)
                # when pathThrough should be FALSE.
                else:
                    pathThrough = (neighbor == self.pathNeighbourThroughToRoot)
                    new_message = Message(self.root, self.distance,
                                        self.switchID, neighbor,
                                        pathThrough, message.ttl - 1)
    ...
    def generate_logstring(self):
        ...
        linkStrings = [f"{self.switchID} - {destinationSwitchID}" for destinationSwitchID in self.activeLinks]
        final_output = ', '.join(linkStrings)
        return final_output
